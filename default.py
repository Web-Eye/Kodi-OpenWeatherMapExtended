...
BASE_URL       = 'https://api.openweathermap.org/data/2.5/%s'
...
def forecast(loc,locid,locationdeg):
    ...
    #daily_string = 'onecall?lat=%s&lon=%s&lang=%s&APPID=%s&exclude=current,minutely,hourly&units=metric' % (lat, lon, lang, APPID)
    
    oneClick_string = 'onecall?lat=%s&lon=%s&lang=%s&APPID=%s&exclude=minutely&units=metric' % (lat, lon, lang, APPID)
    
    retry = 0
    failed = False
    while (retry < 6) and (not MONITOR.abortRequested()):
        oneClick_data = get_data(oneClick_string, 'current')
        log('oneClick_data: %s' % oneClick_data)
        if oneClick_data != '':
            retry = 6
            try:
                oneClick_json = json.loads(oneClick_data)
            except:
                clear()
    ...
    if oneClick_json and oneClick_json != '' :
    
        globalData = {}
        globalData['lat'] = oneClick_json['lat']
        globalData['lon'] = oneClick_json['lon']
    
        if 'current' in oneClick_json:
            current_props(oneClick_json['current'],loc, globalData)
        if 'hourly' in oneClick_json:
            hourly_props(oneClick_json['hourly'])
        if 'daily' in oneClick_json:
            daily_props(oneClick_json['daily'])
    ...
    #    daily_weather = '' 
    #daynum = ''
    #if daily_weather and daily_weather != '' and not 'cod' in daily_weather :
...
def current_props(data,loc, globalData):
...
# standard properties
    ...
    if 'temp' in data:
        set_property('Current.Temperature'      , str(int(round(data['temp']))))
        set_property('Current.DewPoint'         , DEW_POINT(data['temp'], data['humidity'], False))
    else:
        set_property('Current.Temperature'      , '')
        set_property('Current.DewPoint'         , '')
    if 'temp' in data:
        set_property('Current.FeelsLike'        , str(int(round(data['feels_like']))))
    else:
        set_property('Current.FeelsLike'        , '')
    if 'wind_speed' in data:
        set_property('Current.Wind'             , str(int(round(data['wind_speed'] * 3.6))))
        ...
        #set_property('Current.FeelsLike'        , '')
    if 'wind_deg' in data:
        set_property('Current.WindDirection'    , xbmc.getLocalizedString(WIND_DIR(int(round(data['wind_deg'])))))
    else:
        set_property('Current.WindDirection'    , '')
    set_property('Current.Humidity'             , str(data.get('humidity','')))
    set_property('Current.UVIndex'              , str(data.get('uvi','')))
...
# extended properties
    set_property('Current.Cloudiness'           , str(data['clouds']) + '%')
    ...
    if 'F' in TEMPUNIT:
        set_property('Current.Pressure'         , str(round(data['pressure'] / 33.86 ,2)) + ' in')
        if 'sea_level' in data:
            set_property('Current.SeaLevel'     , str(round(data['sea_level'] / 33.86 ,2)) + ' in')
        else:
            set_property('Current.SeaLevel'     , '')
        if 'grnd_level' in data:
            set_property('Current.GroundLevel'  , str(round(data['grnd_level'] / 33.86 ,2)) + ' in')
        else:
        ...
        set_property('Current.Pressure'         , str(data['pressure']) + ' mb')
        if 'sea_level' in data:
            set_property('Current.SeaLevel'         , str(data['sea_level']) + ' mb')
        else:
            set_property('Current.SeaLevel'     , '')
        if 'grnd_level' in data:
            set_property('Current.GroundLevel'      , str(data['grnd_level']) + ' mb')
        else:
            set_property('Current.GroundLevel'  , '')
        rain = 0
        snow = 0
    ...
    if 'wind_gust' in data:
        set_property('Current.WindGust'         , SPEED(data['wind_gust']) + SPEEDUNIT)
    else:
        set_property('Current.WindGust'         , '')
    if 'var_beg' in data:
        set_property('Current.WindDirStart'     , xbmc.getLocalizedString(WIND_DIR(data['var_beg'])))
    else:
        set_property('Current.WindDirStart'     , '')
    if 'var_end' in data:
        set_property('Current.WindDirEnd'       , xbmc.getLocalizedString(WIND_DIR(data['var_end'])))
    else:
    ...
    set_property('Forecast.Latitude'            , str(globalData['lat']))
    set_property('Forecast.Longitude'           , str(globalData['lon']))
    set_property('Forecast.Updated'             , convert_date(data.get('dt','')))
    try:
        set_property('Today.Sunrise'            , convert_date(data['sunrise']).split('  ')[0])
    except:
        set_property('Today.Sunrise'            , '')
    try:
        set_property('Today.Sunset'             , convert_date(data['sunset']).split('  ')[0])
    except:
...
def daily_props(data):
    ...
    for count, item in enumerate(data):
        ...
        set_property('Day%i.Title'              % count, get_weekday(dt, 'l'))
        ...
        # extended properties        
        set_property('Daily.%i.LongDay'         % (count+1), get_weekday(dt, 'l'))
        set_property('Daily.%i.ShortDay'        % (count+1), get_weekday(dt, 's'))
        if DATEFORMAT[1] == 'd' or DATEFORMAT[0] == 'D':
            set_property('Daily.%i.LongDate'    % (count+1), get_month(dt, 'dl'))
            set_property('Daily.%i.ShortDate'   % (count+1), get_month(dt, 'ds'))
        else:
            set_property('Daily.%i.LongDate'    % (count+1), get_month(dt, 'ml'))
            set_property('Daily.%i.ShortDate'   % (count+1), get_month(dt, 'ms'))
        ...
        set_property('Daily.%i.WindDirection'   % (count+1), xbmc.getLocalizedString(int(round(WIND_DIR(item['wind_deg'])))))
        set_property('Daily.%i.WindDegree'      % (count+1), str(item.get('wind_deg','')) + u'°')
        ...
        set_property('Daily.%i.FeelsLike'       % (count+1), FEELS_LIKE(item['temp']['day'], item['wind_speed'] * 3.6, item['humidity']) + TEMPUNIT)
        ...
        set_property('Daily.%i.WindSpeed'           % (count+1), SPEED(item['wind_speed']) + SPEEDUNIT)
        if 'gust' in item: 
        ...
        #weekend properties
        if weekend_count < 2 and get_weekday(item.get('dt',''), 'x') in weekend:
            set_property('Weekend.%i.LongDay'         % (weekend_count+1), get_weekday(dt, 'l'))
            set_property('Weekend.%i.ShortDay'        % (weekend_count+1), get_weekday(dt, 's'))
            if DATEFORMAT[1] == 'd' or DATEFORMAT[0] == 'D':
                set_property('Weekend.%i.LongDate'    % (weekend_count+1), get_month(dt, 'dl'))
                set_property('Weekend.%i.ShortDate'   % (weekend_count+1), get_month(dt, 'ds'))
            else:
                set_property('Weekend.%i.LongDate'    % (weekend_count+1), get_month(dt, 'ml'))
                set_property('Weekend.%i.ShortDate'   % (weekend_count+1), get_month(dt, 'ms'))
            set_property('Weekend.%i.Outlook'         % (weekend_count+1), FORECAST.get(item['weather'][0].get('description',''), item['weather'][0].get('description','')))
            set_property('Weekend.%i.ShortOutlook'    % (weekend_count+1), FORECAST.get(item['weather'][0].get('main',''), item['weather'][0].get('main','')))
            set_property('Weekend.%i.OutlookIcon'     % (weekend_count+1), WEATHER_ICON % weathercode)
            set_property('Weekend.%i.FanartCode'      % (weekend_count+1), weathercode)
            set_property('Weekend.%i.WindDirection'   % (weekend_count+1), xbmc.getLocalizedString(int(round(WIND_DIR(item['wind_deg'])))))
            set_property('Weekend.%i.WindDegree'      % (weekend_count+1), str(item.get('wind_deg','')) + u'°')
            set_property('Weekend.%i.Humidity'        % (weekend_count+1), str(item.get('humidity','')) + '%')
            set_property('Weekend.%i.Cloudiness'      % (weekend_count+1), str(item.get('clouds','')) + '%')
            set_property('Weekend.%i.TempMorn'        % (weekend_count+1), TEMP(item['temp']['morn']) + TEMPUNIT)
            set_property('Weekend.%i.TempDay'         % (weekend_count+1), TEMP(item['temp']['day']) + TEMPUNIT)
            set_property('Weekend.%i.TempEve'         % (weekend_count+1), TEMP(item['temp']['eve']) + TEMPUNIT)
            set_property('Weekend.%i.TempNight'       % (weekend_count+1), TEMP(item['temp']['night']) + TEMPUNIT)
            set_property('Weekend.%i.HighTemperature' % (weekend_count+1), TEMP(item['temp']['max']) + TEMPUNIT)
            set_property('Weekend.%i.LowTemperature'  % (weekend_count+1), TEMP(item['temp']['min']) + TEMPUNIT)
            set_property('Weekend.%i.DewPoint'        % (weekend_count+1), DEW_POINT(item['temp']['day'], item['humidity']) + TEMPUNIT)
            set_property('Weekend.%i.FeelsLike'       % (weekend_count+1), FEELS_LIKE(item['temp']['day'], item['wind_speed'] * 3.6, item['humidity']) + TEMPUNIT)
            if 'F' in TEMPUNIT:
                set_property('Weekend.%i.Pressure'        % (weekend_count+1), str(round(item['pressure'] / 33.86 ,2)) + ' in')
                rain = 0
                snow = 0
                if 'rain' in item:
                    rain = item['rain']
                    set_property('Weekend.%i.Rain'        % (weekend_count+1), str(round(rain * 0.04 ,2)) + ' in')
                else:
                    set_property('Weekend.%i.Rain'        % (weekend_count+1), '')
                if 'snow' in item:
                    snow = item['snow']
                    set_property('Weekend.%i.Snow'        % (weekend_count+1), str(round(snow * 0.04 ,2)) + ' in')
                else:
                    set_property('Weekend.%i.Snow'        % (weekend_count+1), '')
                precip = rain + snow
                set_property('Weekend.%i.Precipitation'   % (weekend_count+1), str(round(precip * 0.04 ,2)) + ' in')
            else:
                set_property('Weekend.%i.Pressure'        % (weekend_count+1), str(item['pressure']) + ' mb')
                rain = 0
                snow = 0
                if'rain' in  item:
                    rain = item['rain']
                    set_property('Weekend.%i.Rain'        % (weekend_count+1), str(int(round(rain))) + ' mm')
                else:
                    set_property('Weekend.%i.Rain'        % (weekend_count+1), '')
                if 'snow' in item:
                    snow = item['snow']
                    set_property('Weekend.%i.Snow'        % (weekend_count+1), str(int(round(snow))) + ' mm')
                else:
                    set_property('Weekend.%i.Snow'        % (weekend_count+1), '')
                precip = rain + snow
                set_property('Weekend.%i.Precipitation'   % (weekend_count+1), str(int(round(precip))) + ' mm')
            set_property('Weekend.%i.WindSpeed'           % (weekend_count+1), SPEED(item['wind_speed']) + SPEEDUNIT)
            if 'gust' in item: 
                set_property('Weekend.%i.WindGust'        % (weekend_count+1), SPEED(item['gust']) + SPEEDUNIT)
            else:
                set_property('Weekend.%i.WindGust'        % (weekend_count+1), '')
    ...
    #       count = 2
    #    
    #    set_property('36Hour.%i.LongDay'         % (count+1), get_weekday(dt, 'l'))
    #    set_property('36Hour.%i.ShortDay'        % (count+1), get_weekday(dt, 's'))
    #    if DATEFORMAT[1] == 'd' or DATEFORMAT[0] == 'D':
    #        set_property('36Hour.%i.LongDate'    % (count+1), get_month(dt, 'dl'))
    #        set_property('36Hour.%i.ShortDate'   % (count+1), get_month(dt, 'ds'))
    #    else:
    #        set_property('36Hour.%i.LongDate'    % (count+1), get_month(dt, 'ml'))
    #        set_property('36Hour.%i.ShortDate'   % (count+1), get_month(dt, 'ms'))
    ...
    #    set_property('36Hour.%i.WindDirection'   % (count+1), xbmc.getLocalizedString(int(round(WIND_DIR(item['wind_deg'])))))
    #    set_property('36Hour.%i.WindDegree'      % (count+1), str(item.get('wind_deg','')) + u'°')
    ...
    #    set_property('36Hour.%i.FeelsLike'       % (count+1), FEELS_LIKE(item['temp']['day'], item['wind_speed'] * 3.6, item['humidity']) + TEMPUNIT)
    ...
    #    set_property('36Hour.%i.WindSpeed'           % (count+1), SPEED(item['wind_speed']) + SPEEDUNIT)
    #    if 'gust' in item: 
...
# extended properties
    for count, item in enumerate(data):
        ...
        weathercode = WEATHER_CODES[code]
        dt = item.get('dt','')
        set_property('Hourly.%i.Time'            % (count+1), get_time(dt))
        if DATEFORMAT[1] == 'd' or DATEFORMAT[0] == 'D':
            set_property('Hourly.%i.LongDate'    % (count+1), get_month(dt, 'dl'))
            set_property('Hourly.%i.ShortDate'   % (count+1), get_month(dt, 'ds'))
        else:
            set_property('Hourly.%i.LongDate'    % (count+1), get_month(dt, 'ml'))
            set_property('Hourly.%i.ShortDate'   % (count+1), get_month(dt, 'ms'))
        ...
        if True:
            if 'wind_deg' in item:
                set_property('Hourly.%i.WindDirection'   % (count+1), xbmc.getLocalizedString(WIND_DIR(int(round(item['wind_deg'])))))
                set_property('Hourly.%i.WindDegree'      % (count+1), str(item['wind_deg']) + u'°')
            else:
                set_property('Hourly.%i.WindDirection'   % (count+1), '')
                set_property('Hourly.%i.WindDegree'      % (count+1), '')
            if 'wind_speed' in item:
                set_property('Hourly.%i.WindSpeed'       % (count+1), SPEED(item['wind_speed']) + SPEEDUNIT)
            ...
            if 'wind_gust' in item:
                set_property('Hourly.%i.WindGust'        % (count+1), SPEED(item['wind_gust']) + SPEEDUNIT)
            else:
        ...
        set_property('Hourly.%i.FeelsLike'           % (count+1), TEMP(item['feels_like']) + TEMPUNIT)
        set_property('Hourly.%i.HighTemperature'     % (count+1), '')
        set_property('Hourly.%i.LowTemperature'      % (count+1), '')
        set_property('Hourly.%i.DewPoint'            % (count+1), TEMP(item['dew_point']) + TEMPUNIT)
        if 'F' in TEMPUNIT:
            set_property('Hourly.%i.Pressure'        % (count+1), str(round(item['pressure'] / 33.86 ,2)) + ' in')
            if 'sea_level' in item:
                set_property('Hourly.%i.SeaLevel'    % (count+1), str(round(item['sea_level'] / 33.86 ,2)) + ' in')
            else:
                set_property('Hourly.%i.SeaLevel'    % (count+1), '')
            if 'grnd_level' in item:
                set_property('Hourly.%i.GroundLevel' % (count+1), str(round(item['grnd_level'] / 33.86 ,2)) + ' in')
            else:
            ...
            set_property('Hourly.%i.Pressure'        % (count+1), str(item['pressure']) + ' mb')
            if 'sea_level' in item:
                set_property('Hourly.%i.SeaLevel'    % (count+1), str(item['sea_level']) + ' mb')
            else:
                set_property('Hourly.%i.SeaLevel'    % (count+1), '')
            if 'grnd_level' in item:
                set_property('Hourly.%i.GroundLevel' % (count+1), str(item['grnd_level']) + ' mb')
            else:
                set_property('Hourly.%i.GroundLevel' % (count+1), '')
            rain = 0
            snow = 0
